		...
		AssetDisk* Load(std::string_view filePath)
		{
			...
			else
			{
				...
				for (uint32_t i = 0; i < numTextures; i++)
				{
					...
					texture.size = { static_cast<float>(importedTexture->mWidth), static_cast<float>(importedTexture->mHeight) };
					...
					uint32_t texelSize = static_cast<uint32_t>(texture.size.GetX()) * static_cast<uint32_t>(texture.size.GetY());
					...
					for (const auto& aiType : SupportedTextureTypes)
					{
						...
						if (textureCount > 0)
						{
							...
							if (currentAIMaterial->GetTexture(aiType, 0, &texturePath) == AI_SUCCESS)
							{
								...
								tex->data = pixels;
								tex->size = { static_cast<float>(width), static_cast<float>(height) }; // NOTE - We don't support 3D textures!

...
void PBRPass::UpdateTransformUniformBuffer(uint32_t frameIndex, TANG::Transform& transform)
{
	// Construct and update the transform UBO
	TANG::TransformUBO ubo{};
	...
	transformUBO[frameIndex].UpdateData(&ubo, sizeof(TANG::TransformUBO));
}
...
{
	TANG::CameraDataUBO ubo{};
	ubo.position = glm::vec4(position, 1.0f);
	ubo.exposure = 1.0f;
	cameraDataUBO[frameIndex].UpdateData(&ubo, sizeof(TANG::CameraDataUBO));
}

void PBRPass::UpdateDescriptorSets(uint32_t frameIndex, std::array<const TANG::TextureResource*, 8>& textures)
{
	// SET 0 - PBR textures
	{
		TANG::DescriptorSet& descSet = pbrDescriptorSets[frameIndex][0];
		TANG::WriteDescriptorSets writeDescSets(0, 8);
	...
	// SET 1 - Projection
	{
		TANG::DescriptorSet& descSet = pbrDescriptorSets[frameIndex][1];
	...
	{
		TANG::DescriptorSet& descSet = pbrDescriptorSets[frameIndex][2];
		TANG::WriteDescriptorSets writeDescSets(3, 0);
...
void PBRPass::Create(const HDRRenderPass* hdrRenderPass, uint32_t swapChainWidth, uint32_t swapChainHeight)
{
	if (wasCreated)
	{
		TANG::LogWarning("Attempting to create pbr pass more than once!");
		return;
	}
...
void PBRPass::Draw(uint32_t frameIndex, const TANG::DrawData& data)
{
	if (!data.IsValid())
	{
		return;
...
{
	TANG::SetLayoutSummary textureLayout(0);
	...
	TANG::SetLayoutSummary projectionLayout(1);
	...
	TANG::SetLayoutSummary volatileLayout(2);
...
void PBRPass::CreateDescriptorSets()
{
	...
	{
		TANG::LogError("Failed to create pbr descriptor set, invalid set layout count! Expected (%u) vs. actual (%u)", 3, pbrSetLayoutCount);
		return;
	}
...
{
	VkDeviceSize transformUBOSize = sizeof(TANG::TransformUBO);
	VkDeviceSize viewUBOSize = sizeof(TANG::ViewUBO);
	VkDeviceSize projUBOSize = sizeof(TANG::ProjUBO);
	VkDeviceSize cameraDataUBOSize = sizeof(TANG::CameraDataUBO);

...
// Used for hashing the "Vertex" struct
namespace std {
...
namespace TANG
{
	...
	private:
		...
		bool LoadAssetResources(std::string_view name)
		{
			...
			const Asset* res = AssetContainer::GetInstance()->GetAsset(name);
			if (res == nullptr)
			{
				LogWarning("Failed to load asset resources! No data was found in the asset container");
				return false;
			}
